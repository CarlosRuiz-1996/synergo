                            ...
                            <thead class="bg-gray-100">
                                <tr>
                                    ...
                                    <th scope="col"
                                        ...
                                        wire:click="order('bnd_catalogo')">
                                        <div class="flex items-center">
                                            <span>Bnd Catalogo</span>
                                            @if ($sort_tabla == 'bnd_catalogo')
                                                @if ($orderBy == 'asc')
                                    ...
                                    <th scope="col"
                                        class="px-3 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider
                                        cursor-pointer"
                                        wire:click="order('n_padre')">
                                        n_padre
                                        @if ($sort_tabla == 'n_padre')
                                            @if ($orderBy == 'asc')
                                                <i class="fas fa-sort-alpha-up-alt mt-1"></i>
                                            @else
                                                <i class="fas fa-sort-alpha-down-alt mt-1"></i>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">
                                            ...
                                            <x-danger-button title="Eliminar"
                                                wire:click="$dispatch('delete-tablas',{{ $catalogo->id }})"><i
    ...
    <x-dialog-modal-xl wire:model.live="open" id="">
        ...
        @slot('content')
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    <x-label-modal>Nombre</x-label>
                        <x-input-modal type="text" class="w-full" wire:model="nombre" />

                        <x-input-error for="nombre" />
                </div>
                <div>
                    <x-label-modal>bnd_catalogo</x-label>
                        <x-input-modal type="text" class="w-full" wire:model="bnd_catalogo" />

                        <x-input-error for="bnd_catalogo" />
                </div>
                <div>
                    <x-label-modal>n_padre</x-label>
                        <x-input-modal type="text" class="w-full" wire:model="n_padre" />

                        <x-input-error for="n_padre" />
                </div>
        ...
        @slot('footer')
            ...
            <x-button wire:click="$dispatch('confirm-tablas',{{ $ctg_id }}) "
