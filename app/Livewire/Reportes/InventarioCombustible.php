...
use App\Exports\ExportInvCombustible;
    ...
    public function encontrarInventario()
    {
        ...
        SalesSum AS (
            SELECT 
                NuCombustible,
                precioventa,
                SUM(venta-JarrasConsigna) AS SumaVenta
        ...
        FinalResults AS (
            ...
            LEFT JOIN ComGasolina g 
                ...
                AND CONVERT(datetime, g.Fecha + ' ' + g.Hora, 120) BETWEEN c.FechaInicio AND c.FechaFin
                AND g.NuCamion <> '1111'
        ...
        SELECT 
            ...
            fr.Precio, 
            CASE 
                WHEN fr.FechaInicio > fr.FechaFin THEN CONVERT(varchar, fr.FechaFin, 103) 
                ELSE CONVERT(varchar, fr.FechaInicio, 103) 
            END AS FechaInicio,
