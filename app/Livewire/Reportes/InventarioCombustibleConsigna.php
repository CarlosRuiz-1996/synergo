...
use App\Exports\reportes\ExcelreporteInvCombustibleconsigna;
...
class InventarioCombustibleConsigna extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.reportes.inventario-combustible-consigna', compact('inv'));
    }
...
SalesSum AS (
    SELECT 
        NuCombustible,
        precioventa,
        SUM(JarrasConsigna) AS SumaVenta
...
InitialInventory AS (
    SELECT 
        NuCombustible,
        ROUND(LitrosConsigna, 0) AS inv_inicial
    FROM InventarioConsigna
    WHERE Fecha >= '2024-03-30' AND Fecha < '2024-04-02'
),
FinalInventory AS (
    SELECT 
        NuCombustible,
        ROUND(LitrosConsigna, 0) AS inv_final
    FROM InventarioConsigna
    WHERE Fecha >= '2024-04-02' AND Fecha < '2024-05-01'
)
SELECT 
    fr.NuCombustible, 
    fr.Descripcion, 
